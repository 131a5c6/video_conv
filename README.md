...
本ツールは、GoogleのAIであるGeminiとの対話を通じて作成した、Windows向け一括動画変換アプリです。
FFmpegを同梱しており、mp4, mkv, mov, avi, webmといった主要な動画用拡張子の変換を行えます。
開発にあたっては、AIがGPUエンコード（NVENC/QSV）への自動対応や、日本語パスでのクラッシュ防止、孤児プロセスの排除など、徹底的な不備修正と堅牢性の向上を実施しました。画質維持やサイズ優先のプリセットを選ぶだけで、誰でも最適な結果が得られます。AIの精密なロジックとFFmpegの汎用性が融合した、実用性の高いツールです。
ダウンロードは、右のReleaseにある最新版からvideo_conv.exeを選択してください。

なお、文章はすべてgeminiにて作成しました。
